...
2. **Mapeo de Duplas (Producto-Cuenta):** Al analizar el historial, no elijas el producto más usado y la cuenta más usada por separado. Debes identificar la **dupla (Producto Odoo + Cuenta Contable)** más frecuente para ese tipo de descripción. Si el historial muestra que 'impresion' siempre va con la cuenta '5101', mantén esa relación y devuélvelos obligatoriamente en el JSON como `producto_odoo_id` y `cuenta_contable_odoo_id`. Si hay un cambio deliberado en las facturas más recientes, sigue la nueva tendencia.
